  ...
  return (
    <section className="not-prose relative pb-8 pt-16">
      <div className="max-w-7xl mx-auto px-4 md:pl-32 md:pr-8">
        ...
        <div className="grid grid-flow-col auto-cols-[78%] sm:auto-cols-[46%] gap-3 overflow-x-auto pb-2 snap-x snap-mandatory md:grid-flow-row md:auto-cols-auto md:grid-cols-5 md:overflow-visible md:pb-0 md:snap-none md:gap-4">
          {otherSections.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="group block no-underline min-w-0 snap-start"
            >
              <div className="relative w-full aspect-[4/3] overflow-hidden rounded-lg bg-[#1f1f29] transition-transform duration-300 md:group-hover:scale-[1.1]">
                <Image
                  src={item.imageSrc}
                  alt={item.label}
                  fill
                  className="object-cover opacity-90 md:opacity-95 md:group-hover:opacity-40 md:group-hover:scale-105 transition-all duration-500"
